...
export const Reports = () => {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const [
        ...
        applyBoth(apiClient.from('test_plans').select('id, generated_by_ai, created_at')),
        applyBoth(apiClient.from('test_cases').select('id, generated_by_ai, created_at')),
        applyBoth(apiClient.from('test_executions').select('id, status, created_at, executed_at')),
        applyBoth(apiClient.from('requirements').select('id, created_at')),
        applyBoth(apiClient.from('defects').select('id, status, severity, created_at')),
      ]);
      ...
      console.error(e);
      toast({ title: 'Erro ao carregar relat�rio', variant: 'destructive' });
    } finally {
      setLoading(false);
    }
  ...
  const handleExport = async () => {
    ...
    if (!hasPermission('can_export')) {
      toast({ title: 'Sem permiss�o para exportar', variant: 'destructive' });
      return;
    }
    setExporting(true);
    try {
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              {currentProject ? currentProject.name : 'Todos os projetos'} � vis�o consolidada
            </p>
          </div>
        </div>
      ...
      {loading ? (
        ...
        <div className="space-y-6">
          ...
          {/* -- Vis�o Geral -- */}
          <section>
            <h2 className="text-sm font-semibold text-muted-foreground uppercase tracking-wider mb-3">Vis�o Geral</h2>
            <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-3">
              ...
              <StatCard label="Execu��es" value={data.overview.totalExecutions} icon={Play} />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="border border-border rounded-lg p-4 bg-card space-y-4">
              <div className="flex items-center justify-between">
                <h2 className="text-sm font-semibold">Resultado das Execu��es</h2>
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-xs text-muted-foreground">aprova��o</span>
                </div>
              </div>
              <div className="space-y-2">
                ...
                <BarRow label="N�o testado" value={data.executions.not_tested} max={data.overview.totalExecutions} color="bg-muted-foreground/30" />
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            {/* Atividade recente */}
            <div className="border border-border rounded-lg p-4 bg-card space-y-3">
              <h2 className="text-sm font-semibold">�ltima Atividade</h2>
