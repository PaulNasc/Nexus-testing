import { apiClient as apiClient } from '@/lib/api';
...
export const logActivity = async (
  ...
  try {
    ...
    if (!uid) return;
    await apiClient.from('activity_logs' as any).insert({ user_id: uid, action, context, metadata });
  } catch (e) {
...
async function ensureProjectNotPaused(projectId?: string) {
  if (!projectId) return;
  const { data, error } = await apiClient.from('projects').select('status').eq('id', projectId).maybeSingle();
...
async function ensureWritableByPlanId(planId?: string) {
  if (!planId) return;
  const { data: plan } = await apiClient.from('test_plans').select('project_id').eq('id', planId).maybeSingle();
...
const cleanupUserNotifications = async (userId: string) => {
  try {
    ...
    const { data: excessNotifs } = await apiClient
      .from('notifications' as any)
      .select('id')
      .eq('user_id', userId)
...
export const getDashboardStats = async (projectId?: string) => {
  ...
  const result = await (apiClient as any).get(path);
...
export const deleteTestPlan = async (id: string) => {
  const { data: row } = await apiClient.from('test_plans').select('id, sequence, title').eq('id', id).maybeSingle();
  const { error } = await apiClient
    .from('test_plans')
    .delete()
    .eq('id', id);
...
export const getPlanLinkedDetails = async (
  ...
  const token = localStorage.getItem('krg_local_auth_token');
  const res = await fetch(`${import.meta.env.VITE_API_URL || '/api'}/db/plan-linked-details/${planId}`, {
    method: 'GET',
    headers: {
      'Content-Type': 'application/json',
      'Authorization': `Bearer ${token}`
  ...
  const body = await res.json();
  if (!res.ok) {
    throw new Error(body.error?.message || 'Erro ao carregar detalhes vinculados do plano');
  }
...
export const deleteTestCase = async (id: string) => {
  const { data: row } = await apiClient.from('test_cases').select('id, sequence, title').eq('id', id).maybeSingle();
  const { error } = await apiClient
    .from('test_cases')
    .delete()
    .eq('id', id);
...
export const updateTestExecution = async (id: string, updates: Partial<TestExecution>): Promise<TestExecution> => {
  ...
  try {
    const { data: row, error: selErr } = await apiClient
      .from('test_executions')
      .select('*')
      .eq('id', id)
      .maybeSingle();
...
export const deleteTestExecution = async (id: string) => {
  const { data: row } = await apiClient.from('test_executions').select('id, sequence, status, executed_at').eq('id', id).maybeSingle();
  const { error } = await apiClient
    .from('test_executions')
    .delete()
    .eq('id', id);
...
export const deleteRequirement = async (id: string) => {
  const { data: row } = await apiClient.from('requirements').select('id, title').eq('id', id).maybeSingle();
  const { error } = await apiClient
    .from('requirements')
    .delete()
    .eq('id', id);
...
export const getRequirementCaseLinksByRequirements = async (userId: string, requirementIds: string[]): Promise<{ requirement_id: string, case_id: string }[]> => {
  ...
  const { data, error } = await withUserScope(
    apiClient.from('requirements_cases').select('requirement_id, case_id'),
    userId
...
export const createDefect = async (defect: Omit<Defect, 'id' | 'created_at' | 'updated_at'>): Promise<Defect> => {
  ...
  // Garante user_id
  if (!payload.user_id) {
    const { data: authData, error: authErr } = await apiClient.auth.getUser();
  ...
  if (!payload.project_id) {
    try {
      ...
      if (payload.case_id) {
        const { data: caseData, error: caseErr } = await apiClient
          .from('test_cases')
          .select('plan_id')
      ...
      if (!planId && payload.execution_id) {
        const { data: execData, error: execErr } = await apiClient
          .from('test_executions')
          .select('plan_id')
      ...
      if (planId) {
        const { data: planData, error: planErr } = await apiClient
          .from('test_plans')
          .select('project_id')
          .eq('id', planId)
          .single();
...
export const deleteDefect = async (id: string) => {
  const { data: row } = await apiClient.from('defects').select('id, title, status').eq('id', id).maybeSingle();
  const { error } = await apiClient
    .from('defects')
    .delete()
    .eq('id', id);
