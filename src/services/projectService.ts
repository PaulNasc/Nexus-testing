...
export const deleteProjectCascade = async (projectId: string): Promise<void> => {
  ...
  const { data: plans, error: plansErr } = await apiClient
    .from('test_plans')
    .select('id')
  ...
  let caseIds: string[] = [];
  if (planIds.length) {
    const { data: cases, error: casesErr } = await apiClient
      .from('test_cases')
      .select('id')
      .in('plan_id', planIds);
  ...
  if (planIds.length) {
    const { data: execs, error: execErr } = await apiClient
  ...
  try {
    const { data: reqs, error: reqErr } = await apiClient
      .from('requirements')
      .select('id')
      .eq('project_id', projectId);
