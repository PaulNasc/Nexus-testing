...
import { notifyStakeholders } from '@/services/apiClientService';
...
export const AIBatchGeneratorForm = ({ onSuccess, type = 'plan', mode = 'standard' }: AIBatchGeneratorFormProps) => {
  ...
  const saveCasesToSupabase = async (cases: Array<Record<string, any>>) => {
    try {
  ...
  const savePlansToSupabase = async (plans: Array<Record<string, any>>) => {
    try {
